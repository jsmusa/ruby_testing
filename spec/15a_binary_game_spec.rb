...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      ...
      it 'completes loop and displays error message once' do
        ...
        max = game_input.instance_variable_get(:@maximum)
        error_message = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input).to receive(:puts).and_return(error_message).once
        game_input.player_input(min, max)
      end
    end
  ...
  describe '#verify_input' do
    ...
    subject(:game) { described_class.new(0, 10) }
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        input = 8
        expect(game.verify_input(min, max, input)).to be(input)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = game.instance_variable_get(:@maximum)
        input = 11
        expect(game.verify_input(min, max, input)).to be(nil)
      end
    end
  end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      ...
      it 'returns 9' do
        max_guesses = game.maximum_guesses
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      expect { game_test.display_turn_order(search) }.to change(game_test, :guess_count).by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search).to receive(:make_guess)
      game_test.display_turn_order(search)
    end
