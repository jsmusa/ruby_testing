...
describe 'lucky numbers with rotate! method' do
  ...
  context 'when rotating the array of lucky numbers' do
    ...
    it 'will change the first value to 7' do
      expect { lucky_numbers.shift }.to change { lucky_numbers.first }.from(3).to(7)
    end

    # remove the 'x' before running this test
    it 'will change the last value to 3' do
      expect { lucky_numbers.push(3) }.to change { lucky_numbers.last }.from(42).to(3)
    end

    # remove the 'x' before running this test
    it 'will change the first value to 7 and last value to 3' do
      expect { lucky_numbers.push(3).shift }.to change { lucky_numbers.last }.to(3).and change { lucky_numbers.first }.to(7)
    end
  end
end
